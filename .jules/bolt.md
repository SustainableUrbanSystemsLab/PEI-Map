...
## 2024-05-24 - Mapbox Feature Properties Invalidation and Caching
**Learning:** Mapbox GL JS allocates completely new `properties` objects every time features are queried via `queryRenderedFeatures`. Because of this, caching the normalization map per properties object using a `WeakMap` acts as a complete cache miss on every subsequent interaction loop. We were still repeatedly executing `toLowerCase().replace(...)` on the same property keys across these newly allocated property objects.
**Action:** When working with Mapbox properties, cache the normalized string of the property key globally using a standard `Map` instead of (or in addition to) caching per property object. This ensures the expensive regex string replacement is executed exactly once per unique property key across the entire application's lifetime, regardless of object reallocation. Additionally, avoid `Object.entries()` inside hot loops to prevent intermediate array allocation, preferring `for...in`.
