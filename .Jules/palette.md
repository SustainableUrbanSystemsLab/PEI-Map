...
## 2024-04-14 - Native Dialog Accessibility
**Learning:** Native `<dialog>` elements do not automatically inherit an accessible name from their content. Furthermore, icon-only close buttons inside these dialogs must have explicit ARIA labels and focus treatments to ensure they are perceivable to screen readers and visible to keyboard users. In Playwright, `focus-visible` styling is not triggered by script focus; simulating keyboard navigation (`Tab`) is required.
**Action:** Always apply `aria-labelledby` linking to a heading ID on `<dialog>` tags. Ensure icon-only close buttons have an `aria-label`, a `title` (for tooltips), and explicit `focus-visible` styling. Use simulated keyboard presses to test `focus-visible` states in automated scripts.
